...
// AdSense: only request an ad when the publisher client is replaced with a real ID.
window.setTimeout(() => {
  try {
    const client = document.querySelector('script[src*="adsbygoogle.js"]')?.getAttribute('src') || '';
